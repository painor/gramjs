...
const PUBLIC_KEYS: Record<string, PublicKey> = {
  "-4344800451088585951": { // Telegram servers #1
    ...
    e: bigInt("010001", 16), // Exponent
  },

  "847625836280919973": { // Telegram servers #2
    // -----BEGIN PUBLIC KEY-----
  ...
  "1562291298945373506": { // Telegram servers #3
    // -----BEGIN PUBLIC KEY-----
  ...
  "-5859577972006586033": { // Telegram servers #4
    // -----BEGIN PUBLIC KEY-----
  ...
  "6491968696586960280": { // Telegram servers #5
    // -----BEGIN PUBLIC KEY-----
  ...
  "6427105915145367799": { // CDN DC-121
    // -----BEGIN RSA PUBLIC KEY-----
  ...
  "2685959930972952888": { // CDN DC-140
    // -----BEGIN RSA PUBLIC KEY-----
 ...
 */
export async function encrypt(fingerprint: BigInteger, data: Buffer) {
  const key = PUBLIC_KEYS[fingerprint.toString()];
  if (!key) {
    return undefined;
  }
  ...
  // rsa module uses rsa.transform.bytes2int(to_encrypt), easier way:s
